    ...
    res.forEach( entry => {
      if(entry.frequency == 'Weekly' || entry.frequency == 'Both'){
        ...
        if(entry.type == 'community'){
          knex('travelNotices')
          .select('*')
          .whereRaw('replace(replace(replace(lower(destination), \'\'\'\', \'\'), \',\', \'\'), \'&\', \'and\') like ? and ((current_date <= "returnDate") and (current_date + INTERVAL \'1 week\' >= "arrivalDate")) order by "arrivalDate"', ['%"'+entry.value.toLowerCase()+'"%'])
          .then(notices => {
            notices.map(notice => {
